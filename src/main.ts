...
class PizzaPOSApp {
  ...
  async loadData() {
    try {
      if (this.isWebMode) {
        ...
        this.products = await invoke('plugin:pizza|get_products');
        this.orders = await invoke('plugin:pizza|get_orders');
      }
    } catch (error) {
  ...
  async completeOrder() {
    ...
    try {
      let newOrder: Order;
      if (this.isWebMode) {
        newOrder = await mockInvoke('plugin:pizza|create_order', { order }) as Order;
      } else {
        newOrder = await invoke('plugin:pizza|create_order', { order }) as Order;
      }
    ...
    try {
      if (this.modalMode === 'add') {
        if (this.isWebMode) {
          ...
          await invoke('plugin:pizza|create_product', { product: this.modalProduct });
        }
        ...
        if (this.isWebMode) {
          ...
          await invoke('plugin:pizza|update_product', { product: this.modalProduct });
        }
  ...
  async handleDeleteProduct(productId: number) {
    ...
    try {
      if (this.isWebMode) {
        ...
        await invoke('plugin:pizza|delete_product', { id: productId });
      }
